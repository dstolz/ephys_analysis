...
%     T      [n x 1] event times, s (t = row/Fs, polarity applied, plus
...
%   intervals of REF.line that overlap the trial (TrialEvents) are filtered
%   by duration and by timeRange (relative to the trial onset) and REF.which
%   picks among them. line "Trial" (or the trial line itself) is the
%   trial's own [TrialOnset TrialOffset].
...
%   whole recording. Each event is assigned the trial that contains it;
%   when MASK is given (a restrictive trial selection), events outside the
%   kept trials are dropped. line "Trial" is the pairing's trial line.
%
...
if scope == "trial"
    ...
    [t, k] = pickEvents(iv, ref, 0);
    trial = NaN(size(t));
    if src.hasTrials && ~isempty(t)
        on = src.trials.TrialOnset; off = src.trials.TrialOffset;
        for j = 1:numel(t)
            r = find(on <= t(j) - ref.offsetSec & off >= t(j) - ref.offsetSec, 1);
            if ~isempty(r); trial(j) = r; end
        end
    end
    if ~isempty(mask)
        keep = isfinite(trial);
...
k = k(o);
if isempty(t)
    error('resolveEvents:NoEvents', '%s: no %s %s event is left after the selection (scope %s, which %s).', ...
        src.name, ref.line, ref.edge, scope, ref.which);
end
end
