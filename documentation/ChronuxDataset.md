...
Chronux itself is **not** part of this repository and is not required to prepare
data — only to analyse it. Install it separately and add it to the path:

```matlab
addpath(genpath('C:\src\chronux_2_12'))
...
cx = ChronuxDataset("D:\out\subj1_extract.mat")  % an IntanDataset.toMat output
...
| --- | --- |
| `"LFP"`, `"MUA"`, `"SPIKE"` | `IntanDataset.deriveSignals(SignalOptions..., dataTypeOut=Signal)` — see [intan2matlab](intan2matlab.md) |
| `"RAW"` | `IntanDataset.readData` — broadband amplifier data at the recording rate, unfiltered (only `keepAmpChannels` and `labelField` apply from `SignalOptions`) |
...
| `"detect"` | `IntanDataset.detectSpikes` on the loaded signal (one element per channel) — threshold crossings, not sorted units |
