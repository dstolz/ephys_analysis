...
| Kilosort4 probe `chanMap` | 0-based | `.bin` rows; the `.bin` channel of each site is `chanMap + 1` |
| `units.ksChannel` | 1-based | among the **sorted** channels (after exclusions) |

Sorting does not match probe sites to channels by their hardware number: a
...
- Artifact periods are erased from sample `round(t0·Fs)` up to but not
  including `round(t1·Fs)` (0-based). So an automatically detected run is
  erased starting one sample after its first flagged sample, and a run exactly one sample long is dropped
...
the written `.bin`, never in the source - replaced with
...
| Background runs | at most `Sorting.MaxConcurrent` (default 1) at a time, so the run stays busy until the last dataset has started, unless the Run tab's **Queue the waiting runs** hands the rest to the monitor; automatic artifact detection runs synchronously in MATLAB before each launch (then cached); closing the app does not stop running Python processes (the Run tab's **Stop runs...** does), but drops the queued ones | [App → Run](EphysPreprocessingApp.md#run) |
