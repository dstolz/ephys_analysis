...
**Optional MATLAB toolbox**: [Chronux](http://chronux.org), only for
running the analyses `ChronuxDataset` prepares data for. A copy is bundled in
[`toolboxes/chronux`](../toolboxes/chronux); add it with
`addpath(genpath(...))`. Preparing the data never calls it
