...
- **TDT: use the epoc values for trials.** Each epoc store is an event line (intervals only); its values (e.g. `Freq`, `Levl`) are available from `ds.Reader.readEpocs()` but no step uses them. Trial pairing and the analysis module's grouping take their parameters from Epsych2 sessions; the epoc values could be a second source of per-trial parameters for Synapse recordings.
...
- **Publish the common-reference and Spikes-erase changes to the wiki.** The reference is now drawn once in the Diagram, the Signals tab picks the signals it goes to (`Signals.LFP_Reference` / `MUA_Reference` / `SPIKE_Reference`), `Spikes.ArtifactMode` replaced `RejectArtifacts` (Spikes can erase the periods before detection), Kilosort4's `do_CAR` is off for a referenced `.bin`, and Visualize's Reference defaults to *As the pipeline*. The Artifacts, Signals, Spikes, Visualize and Diagram pages and their screenshots describe the old behaviour.
- **Give the common reference a config section of its own.** `Artifacts.Reference` (and `ReferenceBadLow` / `ReferenceBadHigh`) set the reference for every step, not only artifact detection, and the controls sit on the Artifacts tab. A `Reference` section (and panel), carried onto the datasets as today, would say so; `ArtifactConfig.Reference` on the datasets could stay where it is.
...
- **Publish the new Visualize tab to the wiki.** The tab was rebuilt around `EphysTraceViewer` / `EphysTraceSource`: it shows the recording, the Sorting `.bin` or a derived signal with sorted units and detected spikes over it, reads only the window shown, and has a toolbar, an overview strip and new mouse / key bindings. The wiki's Visualize page, its `visualize-*.png` screenshots (made by hand; `tools/wiki/wikiScreenshots.m` has no Visualize shot yet) and the API pages (two new classes) describe the old tab. Since then it also draws the digital-input events (over the traces and as TTL rows above them), shades the last run's artifact detection, and lists the mouse / key controls in a "?" window instead of under the controls.
- **Publish the Synthetic tab to the wiki.** Help on the Synthetic tab opens the wiki's `Synthetic-Tab` page (`helpURL.m`), which the wiki (last published from `502771b`, before the tab) does not have yet. The page, a screenshot and the `SyntheticDesign` / `syntheticModel` API pages are needed.
- **Visualize: views wider than one read.** A view is at most `MaxReadSamples` (2^27 samples × channels, about 70 s of 64 channels at 30 kHz) wide, because every draw reads its window at full rate. A min / max pyramid of the recording, built once in the background (or by the Sorting step next to the `.bin`), would let the whole recording be drawn at once and the overview strip show the signal itself.
- **Visualize: say whether the run's artifact file matches the current settings.** With no current Detect / Preview, the orange overlay now shows the periods of `<Name>_artifacts.json` as "detected by the last run", whatever settings that run had. A static, read-only `EphysPipeline.cachedDetection(cfg, d)` sharing `artifactIntervalsFor`'s fingerprint (factored out) would let the status line say whether the current settings would detect the same periods:
...
11. **Manifest viewer** (done 2026-09-23, on branch `manifest-viewer`). Started on 2026-09-18 and left uncommitted in a worktree, then saved as `f6e6ca6` on `worktree-analysis-module`. Finished on top of `83d58c6`.
    ...
    - **Tests:** new suite `test_ManifestViewerApp` (41 checks) and two new checks in `test_EphysPreprocessingApp` (271/271 pass).
