classdef ChronuxDataset < handle
    ...
    %   A ChronuxDataset wraps one recording (an IntanDataset, a .mat written by
    %   IntanDataset.toMat, or a plain matrix) and hands out the exact data
    ...
    %
    %   Chronux is not part of this repository. Install it separately
    %   (http://chronux.org) and put it on the MATLAB path; ChronuxDataset works
    ...
    %   Construction
    %   ------------
    %     cx = ChronuxDataset(ds)                    % an IntanDataset
    ...
    %     "LFP" | "MUA" | "SPIKE"  derived with IntanDataset.deriveSignals
    %                              (SignalOptions is forwarded to it)
    %     "RAW"                    broadband amplifier data at the recording
    %                              rate, through IntanDataset.readData
    %
    %   Typical workflow
    %   ----------------
    ...
    %     is the convention of IntanDataset.readData's t vector.
    ...
    %
    %   See also INTANDATASET, ChronuxDataset.tapersFor, ChronuxDataset.makeParams,
    %   EXTRACT_TRIALS.

    properties
        % Which continuous signal this connector serves. "LFP"/"MUA"/"SPIKE"
        % are derived with IntanDataset.deriveSignals; "RAW" is the broadband
        % amplifier data at the recording rate (IntanDataset.readData).
        Signal (1,1) string {mustBeMember(Signal, ["LFP","MUA","SPIKE","RAW"])} = "LFP"

        % Options forwarded to IntanDataset.deriveSignals when the signal is
    ...
    properties (SetAccess = protected)
        Dataset                              % IntanDataset, or [] for other sources
        ...
        function obj = ChronuxDataset(source, opts)
            ...
            elseif isstring(source) || ischar(source)
                ...
                if endsWith(src, ".mat", 'IgnoreCase', true)
                    ...
                    obj.SourceType = "mat";
                elseif isfolder(src)
                    obj.Dataset    = IntanDataset(src);
                    obj.SourceType = "dataset";
                else
                    error('ChronuxDataset:BadSource', ...
                        ['Source must be an IntanDataset, a recording folder, a ' ...
                         '.mat written by toMat, or a numeric matrix; got "%s".'], src);
                end
            else
                error('ChronuxDataset:BadSource', ...
                    ['Source must be an IntanDataset, a recording folder, a .mat ' ...
        ...
        function requireChronux()
            ...
            if ~ChronuxDataset.hasChronux()
                error('ChronuxDataset:NoChronux', ...
                    ['The Chronux toolbox is not on the MATLAB path. Download it ' ...
                     'from http://chronux.org and add it with addpath(genpath(...)). ' ...
