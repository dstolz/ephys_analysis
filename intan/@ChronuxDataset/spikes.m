...
%                        IntanDataset's kilosortResultsDir). Sample indices are
...
%     Source="detect"    IntanDataset.detectSpikes on the loaded continuous
...
%     DetectOptions  struct of IntanDataset.detectSpikes options ("detect")
...
%   IntanDataset.detectSpikes, ChronuxDataset.toPointProcess.
...
error('ChronuxDataset:NoSpikeSource', ...
    ['No spike times available: this connector has no Kilosort4 results. Pass ' ...
     ...
     'signal with IntanDataset.detectSpikes.']);
end
...
if dir0 == ""
    if isempty(obj.Dataset)
        error('ChronuxDataset:NoResultsDir', ...
            'Source="kilosort" needs ResultsDir (no IntanDataset is attached).');
    end
...
ds = obj.Dataset;
if isempty(ds); ds = IntanDataset(); end     % detectSpikes works on any matrix
