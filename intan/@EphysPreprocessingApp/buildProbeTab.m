...
% Rows 6-7: per-recording channel exclusions and the config's default probe
fg = uigridlayout(left, [2 5]);
fg.Layout.Row = [6 7];
...
exLbl.Layout.Row = 1; exLbl.Layout.Column = 1;
obj.ExcludeChannelsField = uieditfield(fg, "text", ...
    "Placeholder", "e.g. 1,5,32-40 (blank = none)", ...
    "Tooltip", "Saved to the manifest of the dataset selected in the Project table.", ...
    "ValueChangedFcn", @(~,~) obj.onApplyExclude("selected"));
obj.ExcludeChannelsField.Layout.Row = 1; obj.ExcludeChannelsField.Layout.Column = 2;
...
obj.ProbeWriteDefaultCheckBox = uicheckbox(fg, "Text", "Save to manifests", "Value", false, ...
    ...
    "ValueChangedFcn", @(~,~) obj.onConfigChanged());
obj.ProbeWriteDefaultCheckBox.Layout.Row = 2; obj.ProbeWriteDefaultCheckBox.Layout.Column = 4;
...
obj.ShowChanNumbersCheckBox = uicheckbox(ig, "Text", "Show channel numbers", "Value", false, ...
