...
%
%   See also IntanDataset.filterContinuous, IntanDataset.detectArtifacts,
%   IntanDataset.readData.
...
if opts.WindowMs(1) > opts.WindowMs(2)
    error('IntanDataset:detectSpikes:BadWindow', ...
...
if isnan(thrIn)
    switch opts.ThresholdMethod
        ...
        case "absolute"
            error('IntanDataset:detectSpikes:NoThreshold', ...
...
if opts.Filter
    if opts.Band(1) >= opts.Band(2)
        error('IntanDataset:detectSpikes:BadBand', ...
            'Band must be [low high] with low < high; got [%g %g].', ...
            opts.Band(1), opts.Band(2));
    end
    if opts.Band(2) >= Fs/2
        error('IntanDataset:detectSpikes:BandAboveNyquist', ...
...
if any(degenerate)
    warning('IntanDataset:detectSpikes:DegenerateThreshold', ...
