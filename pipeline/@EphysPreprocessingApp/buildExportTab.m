...
obj.ExpEpochNonFiniteDropDown = uidropdown(cg, ...
    ...
    "Tooltip", "An epoch whose recorded samples hold NaN or Inf.", ...
    "ValueChangedFcn", changed);
obj.ExpEpochNonFiniteDropDown.Layout.Row = r; obj.ExpEpochNonFiniteDropDown.Layout.Column = [3 4];
r = r + 1;
lab(cg, "Artifact periods:", r);
obj.ExpEpochArtifactsDropDown = uidropdown(cg, ...
    "Items", {'touching one: drop the epoch', 'touching one: keep it (flagged)'}, ...
    "ItemsData", {'drop', 'keep'}, "Value", 'drop', ...
    "Tooltip", "An epoch whose window touches an artifact period the Signals step erased (the " + ...
        "extract's info.artifacts). The trials table flags it either way (EpochArtifact); drop " + ...
        "leaves it out of the continuous signals, as the analysis does.", ...
    "ValueChangedFcn", changed);
obj.ExpEpochArtifactsDropDown.Layout.Row = r; obj.ExpEpochArtifactsDropDown.Layout.Column = [2 3];
r = r + 1;
