...
r = r + 1;
obj.SigBlankArtifactsCheckBox = uicheckbox(cg, "Text", ...
    "Erase the artifact periods first (a line across each), and record them in every file", "Value", true, ...
    "Tooltip", "BlankArtifacts: before LFP / MUA / SPIKE are derived, the dataset's artifact periods " + ...
        "(the manual ones, plus the automatic detection when the Artifacts tab's ""Erase in the signals"" " + ...
        "is on) become a straight line between the levels on either side, so no filter or resampler " + ...
        "spreads an artifact into the samples around it. The periods are stored in every file " + ...
        "(info.artifacts); the analysis and Export's epochs (Artifact periods: drop) leave out the epochs that touch one. " + ...
        "AUX is not changed.", ...
    "ValueChangedFcn", changed);
...
obj.ConvBadModeDropDown = uidropdown(cg, ...
    ...
    "Tooltip", "badChannels: channels replaced by interpolation from the probe geometry " + ...
        "(the 4 nearest good sites on the same shank; neighbouring columns when there is no probe layout).", ...
    "ValueChangedFcn", changed);
...
obj.ConvBadListField = uieditfield(cg, "text", "Placeholder", "e.g. 3, 17-18", ...
    "Tooltip", "Manual mode: 1-based recording channels, like Keep amp channels; channels not kept are ignored.", ...
    "ValueChangedFcn", changed);
