...
r = r + 1;
lab(cg, "Artifact periods:", r);
obj.SpkArtifactModeDropDown = uidropdown(cg, ...
    "Items", {'Reject the events inside them', 'Erase them before detection (the cleaned recording)', 'Ignore them'}, ...
    "ItemsData", {'reject', 'erase', 'none'}, "Value", 'reject', ...
    "Tooltip", "ArtifactMode. Reject: detect on the recording as read, then drop the events inside a period. " + ...
        "Erase: detect on the cleaned recording - the periods' samples are left out of the thresholds, " + ...
        "the band-pass runs a line across them and no event can lie inside one, so an artifact neither " + ...
        "rings into the samples around it nor raises the threshold. Manual periods always; the automatic " + ...
        "detector's periods when the Artifacts tab's ""Apply to Spikes"" is on.", ...
    "ValueChangedFcn", changed);
obj.SpkArtifactModeDropDown.Layout.Row = r; obj.SpkArtifactModeDropDown.Layout.Column = [2 5];
