...
%buildTrialsTab  Review the pairing of the trials with the trial digital line.
%   The trials come from the dataset's trial source: its Epsych2 session,
%   or, for a TDT block without one, the epocs of the store named as the
%   trial line (EphysDataset.behaviorSource, readBehavior).
...
%   .mat, the trial source / <name>_behavior.mat to the base workspace,
...
top.RowHeight = {30};
top.ColumnSpacing = 6;   % the eight buttons just fit the default 1240 px width
top.Padding = [0 0 0 0];
uilabel(top, "Text", "Dataset:");
obj.TrialsDatasetDropDown = obj.datasetPicker(top);
obj.TrialsLoadButton = uibutton(top, "Text", "Load", ...
...
obj.TrialsPrefetchButton = uibutton(top, "Text", "Prefetch ticked", ...
    "Tooltip", "Read and cache the digital lines of every ticked dataset (Project tab) with a trial source (Epsych2 session or TDT epocs), so Load and the behavior step do not read the recordings again. With Auto approve on, also approve each pairing whose counts match.", ...
...
obj.TrialsEpsychToWorkspaceButton = uibutton(top, "Text", "Trial source to workspace", ...
    "Tooltip", "Load the trial source into the base workspace: the associated Epsych2 session file as saved (Data, Info) as epsych_<name>, or a TDT block's epoc stores as epocs_<name>.", ...
...
% --- row 2: summary --------------------------------------------------------
obj.TrialsSummaryLabel = uilabel(g, "Text", "Scan a project, pick a dataset with a trial source (Epsych2 session or TDT epocs) and press Load.", ...
...
obj.TrialsAutoApproveCheckBox = uicheckbox(sg, "Text", "Auto approve when the counts match", "Value", false, ...
    "Tooltip", "Approve a pairing as soon as it is paired (Load, Prefetch ticked, the behavior step) when the trials and the trial-line intervals are equal in number and nothing is cut. A count mismatch, or a pairing with cuts, still needs review.", ...
    "ValueChangedFcn", changed);
...
h = uilabel(cg, "Text", "Trials"); h.Layout.Row = 2; h.Layout.Column = 1;
...
tips = ["Trials run before the recording started (dropped before pairing).", ...
        "Trials run after the recording stopped (dropped before pairing)."; ...
...
% trial parameter columns and is rebuilt each time it opens.
obj.TrialsTable = uitable(g, "RowName", {}, ...
    ...
    "Tooltip", "Click a header to sort, drag it to move the column. Right-click to add or remove trial parameter columns.", ...
...
% lines (hidden) and Trial labels, the trial parameter values written above
