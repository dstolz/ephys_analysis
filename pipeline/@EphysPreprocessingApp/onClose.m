...
if ~isempty(obj.KSQueue)
    answer = uiconfirm(obj.Fig, ...
        sprintf("%d Kilosort4 run(s) are queued and have not started. Closing the app drops them; ", numel(obj.KSQueue)) + ...
        "the runs already going finish on their own." + newline + newline + ...
        "Their run files are written, so running the Sorting step again starts them." + newline + newline + ...
        "Close anyway?", "Kilosort4 runs queued", ...
        "Options", ["Close anyway", "Stay open"], "DefaultOption", 2, "CancelOption", 2);
    if answer ~= "Close anyway"; return; end
end
