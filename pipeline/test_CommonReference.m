...
%        channel, keepAmpChannels picked after it) from the signals
%        referenceSignals names only (not the LFP by default), erases the
%        artifact periods after it, and derives a referenced and an
%        unreferenced signal from one read exactly as each alone
...
fprintf('\n== 10. the derived signals that take the reference ==\n');
if license('test', 'Signal_Toolbox')
    ...
    [Yd, ~, Id] = dsS.deriveSignals(lfp{:});
    check(isequal(Yd.LFP, xs) && Id.reference.mode == "none" && isempty(Id.reference.signals) ...
        && Id.LFP.reference == "none", ...
        'by default the LFP is taken as recorded (referenceSignals: MUA and SPIKE)');
    refLFP = {'referenceSignals', "LFP"};
    [Yr, ~, Ir] = dsS.deriveSignals(lfp{:}, refLFP{:});
    carS = xs - mean(xs(:, good), 2);
    check(near(Yr.LFP, carS) && Ir.reference.mode == "car" && isequal(Ir.reference.channels, good) ...
        && Ir.reference.signals == "LFP" && Ir.LFP.reference == "car", ...
        'CAR on the LFP: the recording minus the mean of the good channels; info.reference and info.LFP say so');
    [Yk, ~, Ik] = dsS.deriveSignals(lfp{:}, refLFP{:}, keepAmpChannels=[6 2]);
    ...
    [Yn, ~, In] = dsS.deriveSignals(lfp{:}, referenceSignals=strings(1, 0));
    check(isequal(Yn.LFP, xs) && In.reference.mode == "none" && isempty(In.reference.channels), ...
        'referenceSignals=[]: the recording as stored');
    art = [1.0 1.02];
    [Ya, ~, ~] = dsS.deriveSignals(lfp{:}, refLFP{:}, artifactIntervals=art);
    ...
    % Mixed: the LFP as recorded and the SPIKE band referenced, from one read.
    % Each is exactly what it is derived alone, the artifact periods included:
    % the referenced pass erases them again, between the referenced levels.
    spk = {'SPIKE_Fs', Inf, 'artifactIntervals', art};
    [Ymx, ~, Imx] = dsS.deriveSignals('dataTypeOut', ["LFP" "SPIKE"], 'LFP_Fs', Fs, spk{:});
    [Ylo, ~, ~] = dsS.deriveSignals(lfp{:}, 'artifactIntervals', art);
    [Yso, ~, Iso] = dsS.deriveSignals('dataTypeOut', "SPIKE", spk{:});
    check(isequal(Ymx.LFP, Ylo.LFP) && isequal(Ymx.SPIKE, Yso.SPIKE) && Iso.SPIKE.reference == "car" ...
        && Imx.reference.signals == "SPIKE" && Imx.LFP.reference == "none" && Imx.SPIKE.reference == "car", ...
        'LFP as recorded + SPIKE referenced in one call: each exactly as derived alone, periods erased in both');
    [Ymy, ~, Imy] = dsS.deriveSignals('dataTypeOut', ["LFP" "SPIKE"], 'LFP_Fs', Fs, spk{:}, refLFP{:});
    [Yr2, ~, ~] = dsS.deriveSignals(lfp{:}, refLFP{:}, 'artifactIntervals', art);
    [Ys2, ~, ~] = dsS.deriveSignals('dataTypeOut', "SPIKE", spk{:}, 'referenceSignals', strings(1, 0));
    check(isequal(Ymy.LFP, Yr2.LFP) && isequal(Ymy.SPIKE, Ys2.SPIKE) && Imy.SPIKE.reference == "none", ...
        'and the other way round: the SPIKE band as recorded (a copy) next to a referenced LFP');
    errId = '';
    try
        dsS.deriveSignals(lfp{:}, referenceSignals="AUX");
    catch ME
        errId = ME.identifier;
    end
    check(strcmp(errId, 'EphysDataset:deriveSignals:referenceSignals'), 'referenceSignals takes LFP / MUA / SPIKE only');
    aS.Reference = "cmr"; dsS.ArtifactConfig = aS;
    [Ym, ~, Im] = dsS.deriveSignals(lfp{:}, refLFP{:});
