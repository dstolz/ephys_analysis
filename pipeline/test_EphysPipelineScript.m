...
check(contains(txtBg, "maxConcurrent = 2;") && contains(txtBg, "launched = strings(0, 1);") ...
    && contains(txtBg, "d.runSpikeInterface(BeforeLaunchFcn=@() waitForSortingSlot(launched, maxConcurrent), ExtraSettings=ks4") ...
    && contains(txtBg, "launched(end+1) = res.statusFile;"), ...
    'standalone script starts background runs Sorting.MaxConcurrent at a time');
