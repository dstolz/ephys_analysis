...
check(contains(html, "Preprocessing diagram: gui test") ...
    ...
    && contains(html, "+ sorted units (under Sorting)") && startsWith(app.FlowSummaryLabel.Text, "1 of 4"), ...
...
refAt = strfind(full, "data-nav=""ArtRefDropDown,");   % Artifacts, Sorting, Spikes
if numel(refAt) == 3
...
check(count(per, "<div class=""n k-src") == 3 && contains(per, "Downstream (reads step outputs)") ...
    && numel(regexp(per, '<div class="n k-step( dim)?" data-nav=')) == 6 ...
    ...
    'Layout "Tree per step": a tree from the recording for Artifacts, Signals and Spikes; Sorting (under the artifact periods), sorted units and Export downstream; saved as a preference');
