...
% Spike times are on the continuous clock ((sample-1)/Fs): an onset at row r
% (t = r/Fs) is at (r-1)/Fs there, and the spikes are cut around that.
spk0 = (round(onsets * Fs) - 1) / Fs;       % a spike time is (sample-1)/Fs
u1 = [0.095; spk0(1); 0.110; spk0(2) + tPre; spk0(2) + tPost];
...
check(max(abs(E2.units(1).times{1} - (u1(1:3) - spk0(1)))) < 1e-12, ...
    'units: TimeBase "onset" stamps spikes relative to the event');
check(E2.units(1).times{1}(2) == 0, 'units: a spike in the event''s own sample is at exactly 0');
...
check(max(abs(E3.units(1).times{1} - (u1(1:3) - spk0(1) - tPre))) < 1e-12, ...
...
fprintf('\n== 8. a derived signal, and EpochComplete in samples ==\n');
S30 = S;
S30.info.origFs = 30000;                                   % LFP at 1 kHz from a 30 kHz recording
S30.events = struct('din0', [3000 4500; 12000 13500] / 30000);   % recording rows 3000 and 12000
E30 = ds.eventEpochs(Extract=S30, EventSource="line", EventLine="din0", Window=twin, ...
    Units=false, Detected=false);
check(E30.event.eventFs == 30000 && E30.signals.LFP.info.eventFs == 30000 ...
    && isequal(E30.signals.LFP.info.onsetSamples, [101 401]) ...
    && isequal(E30.signals.LFP.data(:, 1, 1), double(X(101 + (s0:s1), 1))), ...
    'row 3000 at 30 kHz lies at 0.09997 s: its epoch is centred on LFP row 101 (0.100 s), not 100');
check(contains(E30.meta.conventions.epochs, "round((onset - 1/eventFs)*Fs) + 1"), ...
    'meta.conventions states the onset rule used');
Ec = ds.eventEpochs(Extract=S, EventSource="times", Times=0.2, Window=[-0.2 0.1], ...
    Units=false, Detected=false);
check(~Ec.trials.EpochComplete && Ec.event.nIncomplete == 1 && Ec.signals.LFP.nIncomplete == 1 ...
    && isnan(Ec.signals.LFP.data(1, 1, 1)) && ~any(isnan(Ec.signals.LFP.data(2:end, 1, 1))), ...
    'a window inside the recording in seconds but not in samples (row 0 is NaN) is not complete');
Es0 = ds.eventEpochs(Extract=S, EventSource="times", Times=0.2, Window=[-0.2 0.1], ...
    Units=false, Detected=false, OnsetRule="sample");
check(Es0.trials.EpochComplete && Es0.signals.LFP.nIncomplete == 0, ...
    'the same window from row 201 ("sample") is complete');
ws = warning('off', 'ChronuxDataset:IncompleteTrials');
Ed2 = ds.eventEpochs(Extract=S, EventSource="times", Times=[0.2; 0.5], Window=[-0.2 0.1], ...
    Units=false, Detected=false, Incomplete="drop");
warning(ws);
