...
check(app.VizDataset == app.Project.Datasets(row1) && strcmp(app.VizArtButton.Enable, 'on') ...
    && any(strcmp(app.VizSourceDropDown.ItemsData, 'recording')) && ~isempty(app.VizData.units) ...
    && app.Viewer.LastRender.error == "" && app.Viewer.LastRender.nSpikes > 0, ...
    'opening Visualize loads the active dataset: its recording, with its sorted units drawn over it');
...
check(app.VizDataset == app.Project.Datasets(row2) && strcmp(app.VizArtButton.Enable, 'on') ...
    && ~contains(app.VizStatusLabel.Text, "The plot shows"), ...
    'with Visualize open, choosing another dataset loads it at once');
app.selectDataset(row1);
...
check(numel(item) == 1 && isequal(item.UserData, row2) && ~item.Checked ...
    && isequal(app.DatasetMenu.Children(1:2), [app.DatasetManifestItem; app.DatasetAllMenu]) ...
